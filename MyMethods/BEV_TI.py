...
for folder in folders:
    ...
    if plot:
        ...
        mask_pd = np.logical_and(mask_pd, TI_radar_points[:, 0] <= 40)
        mask_pd = np.logical_and(mask_pd, TI_radar_points[:, 0] >= -40)
        ...
        # for i1 in range(TI_points.shape[0]):
        #     point = TI_points[i1, :]
        ...
        plt.scatter(TI_points[:, 0], TI_points[:, 1], s=1, marker='o', cmap=cmp, norm=norm)
